...
#
# Get All RxNORM SCDs and SBDs for a THERAPEUTIC CATEGORY - MUSCLE RELAXANTS VERSION
#
...
# MUSCLE RELAXER COVERAGE
# IN = INGREDIENTS FROM FDASPL has_EPC CLASS ID = N0000175737  + BACLOFEN

# https://rxnav.nlm.nih.gov/REST/rxclass/classMembers.json?classId=N0000175737&relaSource=FDASPL&rela=has_EPC&ttys=IN
...
open (my $fh1,'>','MUSCLERELAXANT_SCDSBDGetterOutput.txt');
...
#SOURCE = FDASPL, RELATION = has_EPC, CODE = N0000175737, FOR MUSCLE RELAXANT
my @args = (          '-X', "GET",
...
#ADD BACLOFEN AS INGREDIENT BECAUSE IT IS NOT ON THE RXNORM LIST FOR THIS CLASS N0000175737
push (@ingredientList,1292);

#ADD METAXOLONE AS INGREDIENT BECAUSE IT IS NOT ON THE RXNORM LIST FOR THIS CLASS N0000175737
push (@ingredientList,59078);

print "Ingredient List built \n";


#PRINT INGREDIENT (IN) RXCUIS FROM SINGLE SOURCE, FDASPL
print $fh1 "RXNORM MUSCLE RELAXANT INGREDIENT CODE LIST \n\n";
